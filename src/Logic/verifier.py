...
class Verifier(metaclass=SingletonMeta):
    ...
    def verify_identity(self, id_number: int, first_name: str, surname: str, date_of_birth: str, justification: str, organisation: Organisation) -> bool:
        with record_failures(self.LOG_REPOSITORY, Action.VERIFY, organisation.name, id_number, justification, "identity"):
            required_attributes = {
            ...
            log = Log.for_verify(organisation.name, id_number, validated_justification, "identity", result)
            self.LOG_REPOSITORY.add(log)

            return result

    def verify_minimum_age(self, id_number: int, minimum_age: Any, justification: str, organisation: Organisation) -> bool:
        with record_failures(self.LOG_REPOSITORY, Action.VERIFY, organisation.name, id_number, justification, "minimum_age"):
            ...
            log = Log.for_verify(organisation.name, id_number, validated_justification, "minimum_age", result, str(validated_min_age))
            self.LOG_REPOSITORY.add(log)

            return result

    def verify_attribute(self, id_number: int, attribute: str, claimed_value: str, justification: str, organisation: Organisation) -> bool:
        with record_failures(self.LOG_REPOSITORY, Action.VERIFY, organisation.name, id_number, justification, attribute):
            if attribute not in organisation.verifiable_attributes:
                raise ValueError(f"Access denied: {organisation.name} is not authorised to verify '{attribute}' attribute")
            ...
            log = Log.for_verify(organisation.name, id_number, validated_justification, attribute, result)
            self.LOG_REPOSITORY.add(log)

            return result

    def verify_suspended_in_period(self, start_date: str, end_date: str, id_number: int, justification: str, organisation: Organisation) -> bool:
        with record_failures(self.LOG_REPOSITORY, Action.VERIFY, organisation.name, id_number, justification, "suspended_in_period"):
            ...
            audit_log = Log.for_verify(organisation.name, id_number, validated_justification, "suspended_in_period", result, period_context)
